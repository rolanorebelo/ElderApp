...
const NeighbourList = ({ navigation }) => {
  ...
  const fetchNeighbors = async () => {
    try {
      ...
      neighborsSnapshot.forEach((doc) => {
        ...
        console.log('UsSSS------',user.uid);
        console.log('Neiggggg---------', doc.id);
        if (doc.id !== user.uid) {
          neighborsData.push({
            uid: doc.id,
  ...
  const handleAddRemoveFriend = async (neighbor) => {
    try {
      ...
      if (neighbor.isFriend) {
        ...
        await updateDoc(userRef, { [`friends.${neighbor.uid}`]: null });
        await updateDoc(doc(getFirestore(), 'users', neighbor.uid), { [`friends.${user.uid}`]: null });
      } else {
        // Add friend relationship
        await updateDoc(userRef, { [`friends.${neighbor.uid}`]: true });
        await updateDoc(doc(getFirestore(), 'users', neighbor.uid), { [`friends.${user.uid}`]: true });
      }
  ...
  const renderNeighborItem = ({ item }) => (
    <TouchableOpacity style={styles.neighborItem} onPress={() => item.isFriend && handleChatPress(item)}>
      <Image
        source={
      ...
      <TouchableOpacity style={styles.addRemoveButton} onPress={() => handleAddRemoveFriend(item)}>
