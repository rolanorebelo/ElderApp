...
export default function Chat() {
    ...
    navigation.setOptions({
      ...
      headerLeft: () => (
        <View style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 10 }}>
          <Image source={{ uri: task.profilePicture }} style={{ width: 40, height: 40, borderRadius: 20 }} />
          <Text style={{ fontSize: 16, fontWeight: 'bold', color: 'white', marginLeft: 10 }}>{task.task_description.user_name}</Text>
        </View>
      ),
    });
  }, [navigation, task.profilePicture, task.task_description.user_name]);

  useLayoutEffect(() => {
    const collectionRef = collection(database, 'chats');
  
    if (task.selectedVolunteerId && auth.currentUser.uid) {
      const firstParticipantQuery = query(
        collectionRef,
        orderBy('createdAt', 'desc'),
        where('participants', 'array-contains', task.selectedVolunteerId)
      );
  ...
  useEffect(() => {
    setChatParticipants([task.selectedVolunteerId, auth.currentUser.uid]);
  }, [task]);
  ...
  const onSend = useCallback(
    (messages = []) => {
      ...
      const participants = [auth.currentUser.uid, task.selectedVolunteerId];
    ...
    [auth.currentUser.uid, task.selectedVolunteerId]
  );
